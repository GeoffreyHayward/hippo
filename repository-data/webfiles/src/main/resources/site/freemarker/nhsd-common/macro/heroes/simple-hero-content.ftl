...
<#macro simpleHeroContent options>
    <#assign uiPath = options.uiPath?has_content?then(options.uiPath, "hero") />
    ...
    <#if options.summary?has_content>
        <#if hst.isBeanType(content, 'org.hippoecm.hst.content.beans.standard.HippoHtml')>
            <div class="nhsd-!t-margin-top-6">
                <@hst.html hippohtml=options.summary contentRewriter=brContentRewriter />
            </div>
        <#else>
    ...
    <#if options.buttons?has_content && options.buttons?size gt 0>
        ...
        <nav class="nhsd-m-button-nav nhsd-m-button-nav--condensed ${buttonAlignmentClass} nhsd-!t-margin-top-6">
            <#list options.buttons as button>
                ...
                <a class="nhsd-a-button ${buttonClasses}" href="${button.src}">
